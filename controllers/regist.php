...
class regist extends Controller {
	...
	function __construct() {
			...
			parent::__construct('regist_model');
	...
	function index() {
		...
		$this -> viewLoader -> render('regist/index' ,array('data'=>$data));
	...
	function storeUpdate($id){
	 ...
	 function list($id){
		$reg = $this -> model -> getAllReg($id);
	   
		
		$this->viewLoader->render('regist/list' ,array('reg'=>$reg));
	}
    function deletereg($id) {

		$this -> model -> deleteregist($id);
		header('location:' . BASEPATH . 'regist/');
	}
